...
<body>
  ...
  <?php
  $con=conectar();
  #para sacar los datos de las canciones
  $query="SELECT so.song_id, so.artist,so.title, so.price FROM songs so WHERE so.song_id not in (select song_id from user_songs where user_id = ".$_SESSION['logeado'].")";

  $result = $con->query($query);
  while( $row = $result->fetch_array()){ 
   #recorremos el bucle para cada cancion
   $img = "img/$row[0].jpg";
  ...
  echo    "<a href='#' class='btn mr-2 ' onclick='anadirCancion(".$row[0].")'><i class='fas fa-link' ></i> Comprar </a>";

  echo    "<a href='#' class='btn ' style='margin-left:20px;'><i class='fab fa-github' ><i class='bi bi-play-fill'></i></i></a>";
  echo    "<footer class='blockquote-footer' style='margin-top:auto;'><h6>$row[3]€ <h6></footer>";
  echo     " </blockquote>";
